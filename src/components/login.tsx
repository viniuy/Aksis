import { useState } from 'react'
import { motion } from 'motion/react'
import { Button } from '@/components/ui/button'
import { useStore } from '@/lib/store'

const item = {
  hidden: { opacity: 0, y: 12, filter: 'blur(4px)' },
  show: { opacity: 1, y: 0, filter: 'blur(0px)', transition: { type: 'spring', bounce: 0.2, duration: 0.7 } },
} as const

export function Login() {
  const signIn = useStore((s) => s.signIn)
  const [pending, setPending] = useState(false)
  const start = async () => {
    setPending(true)
    await signIn()
    setPending(false)
  }
  return (
    <motion.main
      className="grid min-h-dvh place-items-center px-5 py-10"
      exit={{ opacity: 0, scale: 0.98, filter: 'blur(4px)' }}
      transition={{ duration: 0.25 }}
    >
      <motion.div
        className="flex w-full max-w-[360px] flex-col gap-3.5"
        initial="hidden"
        animate="show"
        variants={{ show: { transition: { staggerChildren: 0.07 } } }}
      >
        <motion.img variants={item} src="/favicon.svg" alt="" className="mb-2 size-10" />
        <motion.h1 variants={item} className="text-[34px] leading-none font-semibold tracking-[-0.045em]">
          Aksis
        </motion.h1>
        <motion.p variants={item} className="text-muted-foreground">
          Assignments, quizzes and requirements, sorted by what’s due next.
        </motion.p>
        <motion.div variants={item} className="mt-3">
          <Button size="lg" variant="secondary" className="h-11 w-full rounded-xl" disabled={pending} onClick={start}>
            <GoogleIcon />
            Continue with Google
          </Button>
        </motion.div>
      </motion.div>
    </motion.main>
  )
}

function GoogleIcon() {
  return (
    <svg viewBox="0 0 48 48" className="size-[18px]" aria-hidden>
      <path fill="#FFC107" d="M43.6 20.5H42V20H24v8h11.3C33.7 32.7 29.3 36 24 36c-6.6 0-12-5.4-12-12s5.4-12 12-12c3.1 0 5.8 1.2 7.9 3.1l5.7-5.7C34 6.1 29.3 4 24 4 12.9 4 4 12.9 4 24s8.9 20 20 20 20-8.9 20-20c0-1.3-.1-2.4-.4-3.5z" />
      <path fill="#FF3D00" d="m6.3 14.7 6.6 4.8C14.7 15.1 19 12 24 12c3.1 0 5.8 1.2 7.9 3.1l5.7-5.7C34 6.1 29.3 4 24 4 16.3 4 9.7 8.3 6.3 14.7z" />
      <path fill="#4CAF50" d="M24 44c5.2 0 9.9-2 13.4-5.2l-6.2-5.2C29.2 35.1 26.7 36 24 36c-5.2 0-9.6-3.3-11.3-7.9l-6.5 5C9.5 39.6 16.2 44 24 44z" />
      <path fill="#1976D2" d="M43.6 20.5H42V20H24v8h11.3c-.8 2.2-2.2 4.2-4.1 5.6l6.2 5.2C37 39.2 44 34 44 24c0-1.3-.1-2.4-.4-3.5z" />
    </svg>
  )
}
